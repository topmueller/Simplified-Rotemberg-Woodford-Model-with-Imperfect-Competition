...
shocks;
var eta_zeta = 1;   // Preference Shock
end;
...
if exist('GHH_irfs.mat', 'file') == 0
	GHH_irfs = struct;
  //save IRFS
  GHH_irfs.eta_zeta.(['epsilon_mu_' num_1 '_' num_2]) = oo_.irfs;
...
if exist('KPR_irfs.mat', 'file') == 0
	KPR_irfs = struct;
  //save IRFS
	KPR_irfs.eta_zeta.(['epsilon_mu_' num_1 '_' num_2]) = oo_.irfs;
...
subplot(5,3,jj);
plot(1:50, oo_.irfs.([var_names{jj} '_eta_zeta'])(1:50), 'b', 'linewidth', 2); hold on;
xlabel('time');
ylabel('% deviations from S.S.');
if jj==1
@#if preferences
legend(sprintf('GHH preferences with \\epsilon_{\\mu} = %g', epsilon_mu), 'fontSize',10); //add legend
@#else
legend(sprintf('KPR preferences with \\epsilon_{\\mu} = %g', epsilon_mu), 'fontSize',10); //add legend
@#endif
end
